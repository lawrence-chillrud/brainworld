...
def find_most_tumorous_patch(patchified_seg):
    ...
    return list(np.unravel_index(np.argmax(tumour_count), tumour_count.shape))
...
class BrainWorldEnv(Env):
    def __init__(self, grid_size=(4, 4), start_pos=[0, 0], grid_id=[None, None], modality='t1ce', max_steps=20):
        ...
        self.patches = patchify_slice(self.scan[:, :, self.grid_id[1]]) # need to update to specify axis?
        self.patches_seg = patchify_slice(self.seg[:, :, self.grid_id[1]]) # need to update to specify axis?
        
        self.current_pos = start_pos
        self.state = np.expand_dims(self.patches[start_pos[0]][start_pos[1]], axis=-1)
    ...
    def step(self, action):
        ...
        if action == 0: # stay still
            ...
            if self.current_pos == self.goal_pos: # overlap lesion
                done = True
                reward = 1
            else: # outside lesion
                reward = -2
            ...
            if self.current_pos == self.goal_pos: # we were overlapping lesion, but we moved away
                reward = -0.5
            elif np.all(np.array(self.current_pos) + np.array([1, 0]) == self.goal_pos): # we are moving into lesion
                reward = 1
            ...
            if self.current_pos == self.goal_pos: # we were overlapping lesion, but we moved away
                reward = -0.5
            elif np.all(np.array(self.current_pos) + np.array([0, 1]) == self.goal_pos): # we are moving into lesion
                reward = 1
            else: # we weren't in lesion, and we're moving still not in lesion
                reward = -0.5
        ...
        self.state = np.expand_dims(self.patches[self.current_pos[0]][self.current_pos[1]], axis=-1)
    ...
    def reset(self, seed = None, grid_id=[None, None], modality='t1ce'):
        ...
        self.state = np.expand_dims(self.patches[self.start_pos[0]][self.start_pos[1]], axis=-1)
